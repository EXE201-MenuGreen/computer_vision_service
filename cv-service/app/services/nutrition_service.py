"""
NutritionService resolves nutrition data via a 4-tier lookup chain:
  1. In-memory dict cache  (zero latency)
  2. Supabase pgvector     (semantic similarity, ~50-150ms)
  3. USDA FoodData Central (authoritative HTTP API, ~300-800ms)
  4. Hardcoded fallback    (FALLBACK_NUTRITION dict, zero latency)

After a successful USDA lookup (tier 3), the result is stored in Supabase
(fire-and-forget) so future queries hit tier 2 instead.
"""
import asyncio
import httpx
...
from app.services.supabase_client import match_food, upsert_food
...
# Hardcoded fallback per 100g (used when USDA returns nothing).
# Values are approximate; covers Food-101 Western + Vietnamese culinary ingredients.
FALLBACK_NUTRITION: Dict[str, MacroNutrients] = {
    # ── Western / Food-101 defaults ──────────────────────────────────────────
...
def _scale(macro: MacroNutrients, grams: float) -> MacroNutrients:
    ...
    return MacroNutrients(
        ...
        fiber_g=round((macro.fiber_g or 0) * ratio, 1) if macro.fiber_g else None,
    )


def _extract_macros(food_json: dict) -> MacroNutrients:
    """Parse USDA food JSON into MacroNutrients per 100g."""
    nutrients = {n["nutrientId"]: n.get("value", 0)
                 for n in food_json.get("foodNutrients", [])}
    return MacroNutrients(
        calories_kcal=nutrients.get(NUTRIENT_ENERGY, 200),
        protein_g=nutrients.get(NUTRIENT_PROTEIN, 0),
        carbs_g=nutrients.get(NUTRIENT_CARBS, 0),
        fat_g=nutrients.get(NUTRIENT_FAT, 0),
        fiber_g=nutrients.get(NUTRIENT_FIBER),
    )


class NutritionService:
    def __init__(self):
        self._cache: Dict[str, tuple[MacroNutrients, str | None]] = {}

    async def _fetch_nutrition(self, query: str) -> tuple[MacroNutrients, str | None]:
        """
        4-tier nutrition lookup: cache → pgvector → USDA → fallback.

        Vietnamese labels are translated to English via VIET_TO_USDA_QUERY
        before embedding and before the USDA API call.
        """
        ...
        if vector_result is not None:
            self._cache[query] = vector_result
            return vector_result

        # Tier 3: USDA FoodData Central API
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                self._cache[query] = (macros, fdc_id)

                # Store in Supabase for future vector hits (fire-and-forget)
                ...
                return macros, fdc_id

        except Exception as exc:
            logger.warning("usda_lookup_failed", query=query, error=str(exc))

        # Tier 4: hardcoded fallback
        fallback = FALLBACK_NUTRITION.get(query, FALLBACK_NUTRITION["default"])
        self._cache[query] = (fallback, None)
        return fallback, None

    async def lookup_batch(
        self, detected: List[DetectedFood]
    ) -> List[FoodNutrition]:
        results = []
        for food in detected:
            macros_per_100g, fdc_id = await self._fetch_nutrition(food.label)
